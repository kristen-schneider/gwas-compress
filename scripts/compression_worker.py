...
def compress_funnel_format_ints(funnel_format, num_columns, column_data_types, codecs_list, data_type_byte_sizes):
    ...
    for block in funnel_format:
        for column_i in range(len(block)):
            ...
            # compress column
            compress_column_ints(column_as_ints, curr_column_codec, data_type_byte_sizes)


            # int_column_data_types = [1] * num_columns
            # block_header_and_compressed = column_based_compression.compress_block(num_columns, block_as_columns_ints, codecs_list,
            #                                                        int_column_data_types, data_type_byte_sizes)
...
def compress_column_ints(column, column_codec, data_type_byte_sizes):
    ...
    if any(standard_codec in column_codec
        ...
        column_based_compression.compress_with_other(column, column_codec, col_data_type, col_num_bytes)
    # pyfast
    else:
